...
import { 
  ArrowLeft, Filter, ShieldAlert, X, RefreshCw, Check, ChevronDown, 
  ...
  return (
    <div className="relative inline-block ml-1" ref={menuRef}>
      <button 
        ...
        className={`p-1 rounded hover:bg-slate-700 transition-colors ${isActive ? 'text-orange-400 bg-orange-400/10' : 'text-slate-600 hover:text-slate-300'}`}
        title="Filtrar"
      >
          ...
          {hasOptions && (
            <div className="p-2 border-b border-slate-800 bg-slate-950">
              <div className="relative">
                ...
                <input
                  type="text"
                  className="w-full bg-slate-800 border border-slate-700 rounded px-2 py-1.5 pl-7 text-xs text-white focus:outline-none focus:border-orange-500 placeholder-slate-500"
          ...
          <div className="max-h-60 overflow-y-auto p-1 custom-scrollbar">
             {hasOptions ? (
               <>
                 ...
                 >
                    <div className={`w-3.5 h-3.5 border rounded flex items-center justify-center ${filteredOptions.every(o => pendingFilters.includes(o)) && filteredOptions.length > 0 ? 'bg-orange-600 border-orange-600' : 'border-slate-600'}`}>
                 ...
                 {filteredOptions.map(opt => {
                   ...
                   return (
                     <div key={opt} onClick={() => toggleOption(opt)} className="flex items-center gap-2 px-2 py-1.5 hover:bg-slate-800 cursor-pointer rounded">
                       <div className={`w-3.5 h-3.5 border rounded flex items-center justify-center ${isChecked ? 'bg-orange-600 border-orange-600' : 'border-slate-600'}`}>
          ...
          <div className="p-2 border-t border-slate-800 flex justify-between gap-2 bg-slate-950 rounded-b-lg">
            ...
            >
             ...
             <button 
              ...
              className="px-3 py-1 text-[10px] bg-orange-600 text-white rounded hover:bg-orange-500 transition-colors font-medium"
            >
              OK
            </button>
          </div>
      ...
      <div className={`shrink-0 border-slate-800/50 transition-all duration-300 ${isFullscreen ? 'p-2 border-b-0 bg-slate-900/50 backdrop-blur-sm fixed top-0 left-0 right-0 z-50' : 'p-4 md:p-6 border-b'}`}>
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-4 animate-in fade-in duration-300">
              ...
              <div>
                <h1 className="text-2xl font-bold flex items-center gap-2 text-white">
                  <ShieldAlert className="text-orange-500" /> Controle HACCP
                </h1>
                <p className="text-slate-400 text-sm flex items-center gap-2">
                   Visão Geral
                   {/* CONTADOR NO TÍTULO */}
                   <span className="inline-block w-1 h-1 bg-slate-500 rounded-full"></span>
                   <span className="text-orange-400 font-medium">{loading ? "..." : dados.length} registros</span>
                </p>
              </div>
            </div>
          )}
          ...
          <div className={`flex items-center gap-2 flex-wrap ${isFullscreen ? 'w-full justify-between' : ''}`}>
            ...
            <div className="flex items-center gap-2 px-4 py-2 bg-orange-600 rounded-lg text-white font-bold shadow-lg shadow-orange-500/10 ring-1 ring-orange-500 mr-2 animate-in slide-in-from-right-4 duration-500">
               <Hash size={16} className="text-orange-200"/>
               <div className="flex flex-col leading-none">
                 <span className="text-[10px] uppercase text-orange-200 font-medium">Total</span>
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="relative" ref={menuColunasRef}>
                <button onClick={() => setMenuColunasAberto(!menuColunasAberto)} className="flex items-center gap-2 px-3 py-2 bg-slate-900 border border-slate-800 hover:border-orange-500 rounded-lg text-sm font-medium text-slate-300 transition-colors">
                  ...
                  <div className="absolute right-0 top-full mt-2 w-64 bg-slate-900 border border-slate-700 rounded-xl shadow-2xl z-50 p-2 max-h-[400px] overflow-y-auto custom-scrollbar">
                     ...
                     {todasColunas.map(col => (
                        <button key={col} onClick={() => setColunasOcultas(prev => prev.includes(col) ? prev.filter(c => c !== col) : [...prev, col])} className="flex items-center w-full gap-3 p-2 hover:bg-slate-800 rounded text-left group">
                          <div className={`w-3.5 h-3.5 rounded border flex items-center justify-center transition-colors ${!colunasOcultas.includes(col) ? 'bg-orange-600 border-orange-600' : 'border-slate-600 group-hover:border-slate-500'}`}>
              ...
              <button onClick={toggleFullscreen} className={`p-2 rounded-lg transition-all ${isFullscreen ? 'bg-orange-600 text-white shadow-lg shadow-orange-500/20' : 'bg-slate-900 hover:bg-slate-800 border border-slate-800'}`} title="Tela Cheia (F11)">
              ...
              {!isFullscreen && (
                <>
                  ...
                  <button onClick={() => navigate("/graficos-haccp")} className="p-2 bg-slate-800 hover:bg-slate-700 border border-slate-700 rounded-lg text-purple-400"><BarChart3 size={18}/></button>
                  <button onClick={() => fetchDados(filtrosAtivos)} className="p-2 bg-orange-600 hover:bg-orange-500 rounded-lg text-white shadow-lg"><RefreshCw size={18} /></button>
                </>
              )}
            </div>
          </div>
      ...
      <div className={`flex-1 overflow-hidden px-4 md:px-6 pb-6 ${isFullscreen ? 'pt-16' : 'pt-2'}`}>
        <div className="bg-slate-900 rounded-xl border border-slate-800 shadow-xl h-full flex flex-col overflow-hidden relative">
          <div className="overflow-auto flex-1 w-full scrollbar-thin scrollbar-thumb-slate-700 scrollbar-track-transparent bg-slate-950" onScroll={handleScroll}>
            {loading ? (
              <div className="flex h-full items-center justify-center text-slate-500"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-orange-500 mr-3"></div>Carregando...</div>
            ) : (
                ...
                <thead className="text-xs uppercase bg-slate-950 text-slate-400 font-bold sticky top-0 z-20 shadow-md">
                  <tr>
                    {tableStructure.headerRow1.map((item, idx) => (
                      item.type === 'group' ? (
                        <th key={`g-${idx}`} colSpan={item.colSpan} className="px-4 py-2 border border-slate-800 bg-slate-900 text-center text-orange-400 uppercase tracking-wider text-xs border-b-2 border-b-slate-800">
                          {item.name}
                        </th>
                      ) : (
